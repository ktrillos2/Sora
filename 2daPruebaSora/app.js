...
let materiasCanceladas =
  JSON.parse(localStorage.getItem("materiasCanceladas")) ?? [];
agregarMateria();
let pruebaEditadas = [];
...
function agregartodo() {
  ...
  asignaturas.forEach((element) => {
    tab += `
        ...
        <th style="background-color: ${element.color};">${
      element.definitiva
    }</th>
        <th><button type="button" onclick="cancelar(${
          element.id
        })">Cancelar</button><button type="submit" class="my-2 ml-2" id="botonEditar" onclick="editar(${
      element.id
    })">Editar</button></th>
        </tr>
        </tbody>
...
function cancelar(id) {
  ...
  localStorage.setItem(
    "materiasCanceladas",
    JSON.stringify(materiasCanceladas)
  );
...
function definitivatot() {
  asignaturas.map((element) => {
    let multi =
      Number(element.primer) + Number(element.segund) + Number(element.tercer);
    let divi = multi / 3;
