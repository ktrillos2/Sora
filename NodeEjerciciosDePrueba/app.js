...
let crearMovimientos = () => {
    ...
    let Dato = document.getElementById('movimientos').value.toLowerCase().split('');
    Dato.forEach((item) => {
        if (item === "r") {
...
let mayorQue = () => {
    ...
    if (ariba.length > derecha.length && abajo.length && izquierda.length) {
        ...
        console.log("la posicion con mas movimientos hasta el punto de arranque es", contador)
    }
